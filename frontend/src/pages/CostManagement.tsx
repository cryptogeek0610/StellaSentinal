        ...
        <div className="space-y-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Proactive replacement planning based on fleet battery health</p>
            </div>
            {batteryForecast && batteryForecast.forecasts.length > 0 && (
              <button
                onClick={() => {
                  if (!batteryForecast) return;

                  // Create procurement-friendly CSV with clear column names
                  ...
                  // Column headers - procurement-focused
                  const headers = [
                    'Device Model',
                    'Unit Cost ($)',
                  ...
                  // Data rows sorted by immediate need
                  ...
                  for (const f of sortedForecasts) {
                    ...
                    const notes = immediateQty > 0 ? 'URGENT - Order Now' :
                                  f.devices_due_next_month > 0 ? 'Plan for next month' : '';
                  ...
                  // Footer with totals
                  lines.push('');
                  lines.push(`TOTAL,,-,${batteryForecast.total_replacements_due_30_days},${batteryForecast.total_replacements_due_90_days},$${batteryForecast.total_estimated_cost_30_days.toFixed(2)},$${batteryForecast.total_estimated_cost_90_days.toFixed(2)},${batteryForecast.total_devices_with_battery_data},`);
          ...
          {batteryLoading ? (
            ...
            <>
              {/* Summary Cards - Visual hierarchy: urgent left, context right */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-800/60 p-5 rounded-xl border border-slate-700/50">
                  ...
                  <div className="text-xs text-slate-500 mt-2">{batteryForecast.forecasts.length} device models</div>
                </div>
              ...
              {/* Forecast Table - Clean, scannable */}
                ...
                <table className="min-w-full">
                  ...
                  <tbody className="divide-y divide-slate-700/30">
                    {batteryForecast.forecasts
                      ...
                      .map((forecast) => {
                        const healthPercent = Math.min(100, Math.round((forecast.avg_battery_age_months / forecast.battery_lifespan_months) * 100));
                        ...
                        return (
                          <tr key={forecast.device_model} className="hover:bg-slate-700/20 transition-colors">
                            ...
                            <td className="px-4 py-4 text-center">
                              <span className="text-sm font-medium text-slate-200 tabular-nums">{forecast.device_count}</span>
                            </td>
                            ...
                            <td className="px-4 py-4 text-center">
                              <span className="text-sm text-slate-300 tabular-nums">{forecast.devices_due_next_month || '-'}</span>
                            </td>
                            <td className="px-4 py-4 text-center">
                              <span className="text-sm text-slate-300 tabular-nums">{forecast.devices_due_in_90_days}</span>
                            </td>
                            <td className="px-4 py-4 text-right">
                              <span className={`text-sm font-medium tabular-nums ${forecast.estimated_cost_30_days > 0 ? 'text-amber-400' : 'text-slate-500'}`}>
                                {forecast.estimated_cost_30_days > 0 ? formatCurrency(forecast.estimated_cost_30_days) : '-'}
                              </span>
                            </td>
              ...
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  {batteryForecast.data_quality === 'real' && (
                  ...
                  {batteryForecast.data_quality === 'mixed' && (
                    <span className="px-2 py-1 bg-cyan-500/20 text-cyan-400 rounded-md flex items-center gap-1.5">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      Mixed: {batteryForecast.devices_with_health_data} devices with real data
                    </span>
                  )}
                  {batteryForecast.data_quality === 'estimated' && (
                ...
                <span className="text-slate-500">
                  Generated {new Date(batteryForecast.forecast_generated_at).toLocaleDateString(undefined, {
            ...
            <div className="text-center py-16 bg-slate-800/30 rounded-xl border border-slate-700/50">
              <div className="text-slate-400 mb-2">
                <svg className="w-12 h-12 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01M5.07 19H19a2 2 0 001.75-2.97l-7-12a2 2 0 00-3.5 0l-7 12A2 2 0 005.07 19z" />
                </svg>
              </div>
              <p className="text-slate-400 font-medium">No battery forecast data available</p>
              <p className="text-slate-500 text-sm mt-1">Add battery lifespan information to hardware costs to enable forecasting</p>
            </div>
          )}
        </div>
      )}
