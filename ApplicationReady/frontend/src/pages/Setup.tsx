/**
 * Setup Wizard - Environment Configuration
 *
 * A multi-step wizard for configuring the .env file
 * with all necessary settings for the application.
 */
...
const STEPS = [
  {
    id: 'welcome',
    title: 'Welcome',
    description: 'Get started with Stella Sentinel',
    icon: (
      ...
      case 'welcome':
        return (
          <div className="text-center py-8">
            <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-amber-500 via-amber-400 to-orange-500 flex items-center justify-center shadow-stellar">
            ...
            <h2 className="text-2xl font-bold text-white mb-3">Welcome to Stella Sentinel</h2>
            <p className="text-slate-400 max-w-md mx-auto mb-8">
              This wizard will help you configure your environment settings.
              We'll walk through each configuration section step by step.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto text-left">
              <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                ...
                <h3 className="text-white font-medium mb-1">Databases</h3>
                <p className="text-xs text-slate-500">Configure PostgreSQL and SQL Server connections</p>
              </div>
              <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                ...
                <p className="text-xs text-slate-500">Set up LLM for intelligent insights</p>
              </div>
              <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
                ...
                <p className="text-xs text-slate-500">Configure API keys and access control</p>
              </div>
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
            <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-emerald-400 font-medium">Configuration Complete</p>
                  <p className="text-xs text-slate-400 mt-1">
                    Review your settings below and save to generate the .env file.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <SummaryCard
                title="Environment"
                items={[
                  { label: 'Mode', value: config.app_env },
                  { label: 'Metrics', value: config.enable_metrics ? 'Enabled' : 'Disabled' },
                  { label: 'OpenTelemetry', value: config.enable_otel ? 'Enabled' : 'Disabled' },
                ]}
              />
              <SummaryCard
                title="Backend Database"
                items={[
                  { label: 'Host', value: `${config.backend_db_host}:${config.backend_db_port}` },
                  { label: 'Database', value: config.backend_db_name },
                  { label: 'User', value: config.backend_db_user },
                ]}
              />
              <SummaryCard
                title="XSight Database"
                items={[
                  { label: 'Host', value: config.dw_db_host ? `${config.dw_db_host}:${config.dw_db_port}` : 'Not configured' },
                  { label: 'Database', value: config.dw_db_name },
                  { label: 'Trust Cert', value: config.dw_trust_server_cert ? 'Yes' : 'No' },
                ]}
              />
              <SummaryCard
                title="LLM / AI"
                items={[
                  { label: 'Provider', value: config.llm_base_url.includes('ollama') ? 'Ollama' : config.llm_base_url.includes('openai') ? 'OpenAI' : 'Custom' },
                  { label: 'Model', value: config.llm_model_name },
                  { label: 'Endpoint', value: config.llm_base_url },
                ]}
              />
              <SummaryCard
                title="Security"
                items={[
                  { label: 'API Key', value: config.require_api_key ? 'Required' : 'Not required' },
                  { label: 'Trust Headers', value: config.trust_client_headers ? 'Yes (dev only)' : 'No' },
                  { label: 'Default Role', value: config.default_user_role },
                ]}
              />
              <SummaryCard
                title="Streaming"
                items={[
                  { label: 'Status', value: config.enable_streaming ? 'Enabled' : 'Disabled' },
                  { label: 'Redis', value: config.enable_streaming ? config.redis_url : 'N/A' },
                  { label: 'Buffer Size', value: config.enable_streaming ? config.stream_buffer_size.toString() : 'N/A' },
                ]}
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <button
                ...
                className="flex-1 btn-stellar flex items-center justify-center gap-2"
              >
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">Configure your environment settings</p>
      </div>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div key={item.label} className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-300 font-mono truncate ml-2 max-w-[60%] text-right">
              {item.value}
            </span>
          </div>
        ))}
