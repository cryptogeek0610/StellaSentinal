...
def _migrate_results_schema(engine) -> None:
    ...
    def _table_exists(conn, table: str) -> bool:
        row = conn.execute(text("SELECT to_regclass(:table)"), {"table": table}).scalar()
        return row is not None
    ...
    def _ensure_column(
        ...
        if column in columns:
            return
        ddl = f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {column} {column_type}"
        if default_sql is not None:
            ddl += f" DEFAULT {default_sql}"
        conn.execute(text(ddl))
        if default_sql is not None:
    ...
    with engine.begin() as conn:
        if _table_exists(conn, "anomaly_results"):
            _ensure_column(conn, "anomaly_results", "tenant_id", "VARCHAR(50)", "'default'")

        if _table_exists(conn, "device_metadata"):
            _ensure_column(conn, "device_metadata", "tenant_id", "VARCHAR(50)", "'default'")
            _ensure_column(conn, "device_metadata", "os_version", "VARCHAR(50)")
            _ensure_column(conn, "device_metadata", "agent_version", "VARCHAR(50)")

        if _table_exists(conn, "investigation_notes"):
            _ensure_column(conn, "investigation_notes", "tenant_id", "VARCHAR(50)", "'default'")

        if _table_exists(conn, "troubleshooting_cache"):
            _ensure_column(conn, "troubleshooting_cache", "tenant_id", "VARCHAR(50)", "'default'")
