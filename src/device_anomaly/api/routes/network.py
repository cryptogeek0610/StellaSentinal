...
async def get_network_summary(
    ...
    # 1. Get device IDs in the group (including descendants)
    # 2. Query XSight tables filtered by those device IDs
    # 3. Aggregate and return metrics
    logger.info(
        "Network summary requested for tenant %s, group %s (live mode, no data available)", tenant_id, device_group_id
    )
    return NetworkSummaryResponse(
        tenant_id=tenant_id,
        device_group_id=device_group_id,
        ...
        avg_drop_rate=0.0,
        fleet_network_score=100.0,
        wifi_vs_cellular_ratio=0.0,
        devices_in_dead_zones=0,
        recommendations=["No network data available - connect to data source"],
...
async def get_ap_quality(
    ...
    logger.info(
        "AP quality requested for tenant %s, group %s (live mode, no data available)", tenant_id, device_group_id
    )
...
async def get_per_app_usage(
    ...
    logger.info(
        "Per-app usage requested for tenant %s, group %s, period %d days (live mode, no data available)",
        tenant_id,
        device_group_id,
        period_days,
    )
...
async def get_carrier_stats(
    ...
    logger.info(
        "Carrier stats requested for tenant %s, group %s (live mode, no data available)", tenant_id, device_group_id
    )
